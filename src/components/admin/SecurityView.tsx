  ...
  return (
    <div className="space-y-6" id="security-manager">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-[#121216] border border-white/5 p-5 rounded-xl shadow-lg space-y-5">
          ...
          <div className="flex items-start justify-between gap-4">
            <div className="flex-1">
              <span className="text-xs font-bold text-white block">Auto-Lock Workstation Desk</span>
              <span className="text-[10px] text-white/40 leading-normal mt-0.5 block">
                Automatically lock the active workstation to secure sensitive GDS/TDS calculations on inactivity.
              </span>
              
              {sessionAutoLock && (
                <div className="flex items-center gap-2 mt-2">
                  <span className="text-[10px] text-white/50">Lock threshold:</span>
                  <input 
                    type="number" 
                    min={1} 
                    max={120}
                    ...
                    className="w-14 bg-[#181820] border border-white/5 rounded px-2 py-0.5 text-xs text-white outline-none font-mono"
                  />
          ...
          <div className="flex items-start justify-between gap-4">
            <div className="flex-1">
              <span className="text-xs font-bold text-white block">Immutable Operations Audit Pipeline</span>
              <span className="text-[10px] text-white/40 leading-normal mt-0.5 block">
        ...
        <div className="bg-[#121216] border border-white/5 p-5 rounded-xl shadow-lg space-y-5">
          <div className="flex items-center gap-2 border-b border-white/5 pb-3">
            <Smartphone className="w-4.5 h-4.5 text-[#6fa3b8]" />
            <h4 className="text-xs font-bold text-[#eeeef2] uppercase tracking-wider">Advanced Auth Guards</h4>
          </div>
          ...
          <div className="flex items-start justify-between gap-4">
            <div className="flex-1">
              <span className="text-xs font-bold text-white block">Device Whitelisting Constraints</span>
              <span className="text-[10px] text-white/40 leading-normal mt-0.5 block">
      ...
      <div className="bg-[#121216] border border-white/5 p-5 rounded-xl shadow-lg">
        <div className="flex items-center justify-between border-b border-white/5 pb-3 mb-4">
          <div className="flex items-center gap-2">
            <ShieldAlert className="w-4.5 h-4.5 text-red-400" />
            <h4 className="text-xs font-bold text-[#eeeef2] uppercase tracking-wider">Brokerage Security Warnings</h4>
          </div>
        ...
        {weakPinUsers.length > 0 ? (
          <div className="bg-amber-500/5 border border-amber-500/20 rounded-xl p-4 flex items-start gap-3.5">
            ...
            <div>
              ...
              <div className="flex items-center gap-3 mt-3">
                <span className="text-[10px] font-mono text-white/50">Vulnerable nodes: {weakPinUsers.map(u => `${u.first} ${u.last}`).join(", ")}</span>
              </div>
            </div>
          </div>
        ) : (
          <div className="bg-emerald-500/5 border border-emerald-500/20 rounded-xl p-4 flex items-center gap-3">
            ...
            <div className="text-xs">
              ...
              <span className="text-white/40 mt-0.5 block">No simple vulnerable PINs are active in the CRM data layer.</span>
            </div>
          </div>
        )}
      </div>
      ...
      <div className="bg-[#111115] border border-white/5 rounded-xl overflow-hidden shadow-lg">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse min-w-[750px]">
            <thead>
              <tr className="border-b border-white/5 bg-[#121217] text-[9px] text-white/40 uppercase font-black tracking-wider select-none">
              ...
              {deviceSessions.map((session) => (
                <tr key={session.id} className="hover:bg-white/[0.01] transition-all">
                  ...
                  <td className="px-5 py-3">
                    <span className="font-bold text-[#eeeef2]">{session.user}</span>
                  </td>
                  ...
                  <td className="px-5 py-3">
                    <div className="flex items-center gap-2 text-[11px]">
                      ...
                      <span>{session.device} • <span className="text-white/40">{session.os}</span></span>
                    </div>
                  </td>
                  ...
                  {/* Location */}
                  <td className="px-5 py-3 font-semibold text-white/60">
                    {session.location}
                  </td>

                  {/* Timestamp */}
                  <td className="px-5 py-3 text-white/40 font-mono text-[10px]">
                    {session.time}
                  </td>
                    ...
                    {session.status === "authorized" ? (
                      ...
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <button
                          ...
                          className="text-[9px] text-red-400 border border-red-500/20 bg-red-500/10 hover:bg-red-500/20 px-2 py-0.5 rounded uppercase font-black transition-all"
                        >
                          Revoke Access
                        </button>
                      </div>
